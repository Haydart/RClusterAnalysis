...
#data2 <- data[, -which(names(data) %in% c("class"))]
#labels <- iris[,5]
...
intCriteria(as.matrix(sapply(data[, -which(names(data) %in% c("class"))], as.numeric)), clusters$cluster, c("Dunn", "Davies_Bouldin", "Silhouette"))
